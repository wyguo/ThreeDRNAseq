          ...
          column(width = 9,
                 box(title= NULL,
                     ...
                     column(12,
                            ...
                            # downloadButton(outputId = 'plotISnumber_save',label = 'Save',
                            #                class="btn btn-primary",
                            #                style="color: #fff; background-color: #428bca; border-color: #2e6da4;float:left;margin-top: 25px; 
                            #                margin-left: 25px")
                            actionButton(inputId = 'plotISnumber_save',label = 'Save',icon = icon('download',lib = 'font-awesome'),
                                         style="color: #fff; background-color: #428bca; border-color: #2e6da4;
                                         float: left; margin-top: 25px; margin-left: 25px"),
                            bsTooltip(id = "plotISnumber_save", 
                            ...
                            actionButton(inputId = 'plotISnumber_save_view',label = 'View saved plot',
                                         icon = icon('eye',lib = 'font-awesome'),
                                         style="color: #fff; background-color: #428bca; border-color: #2e6da4; 
                                         float: left; margin-top: 25px; margin-left: 25px"),
                            bsTooltip(id = "plotISnumber_save_view",
          ...
          column(width = 9,
                 box(title= NULL,
                     ...
                     bsTooltip(id = "save_tsis_plot", 
                               title = 'Save plot to working directory. If App is not running locally, figures can be downloaded in the last page "Generate report"',
                               placement = "bottom", options = list(container = "body")),
                     actionButton(inputId = 'save_tsis_plot_view',label = 'View saved plot',
                                  icon = icon('eye',lib = 'font-awesome'),
                                  style="color: #fff; background-color: #428bca; border-color: #2e6da4; 
                                  float: left; margin-top: 25px; margin-left: 25px"),
                     bsTooltip(id = "save_tsis_plot_view",
                               title = "Preview saved plots to correct width and height.",
                               placement = "bottom", options = list(container = "body"))
                     ),
                 box(title= 'Plot all significant isoform switches',
                     ...
                     radioButtons(inputId = 'tsis.multiple.plot.format',label = 'Format',
                                  choices = c('both','png','pdf'),selected = 'both',inline = T),
                     actionButton(inputId = 'plot_all_IS_btn',label = 'Plot all',
                                  style="color: #fff; background-color: #428bca; border-color: #2e6da4;"),
                     br(),
                     br(),
                     HTML('Note: (1) The plot width, height and png plot resolution are taken from the above isoform switch visualisation plot. 
                          (2) It may take quite a while to save all the plots.')
                     )
                 )
        ),
        fluidRow(
