...
## Method
The 3D RNA-seq App was developed for rapid and accurate differential expression (DE), differential alternative splicing (DAS) gene and differential transcript usage (DTU) (3D) analysis (Calixto et al., 2018).
...
At the alternative splicing level, DAS genes were determined by comparing each individual transcript $L_2FC$ to gene level $L_2FC$, which was calculated as the weighted average (weights were based on their standard deviation) of $L_2FCs$ of all transcripts of the gene. Then p-values of individual transcript comparison were summarised to a single gene level p-value with `r params$DDD.data$params_list$DAS_pval_method`. A gene was significantly DAS in a contrast group if it had an adjusted p-value < `r params$DDD.data$params_list$pval_cut` and any of its transcript had a $\Delta$ Percent Spliced ($\Delta$PS) ratio $\geq$ `r params$DDD.data$params_list$deltaPS_cut` (Figure 1). For DTU transcripts, the $L_2FC$ of a transcript was compared to the weighted average of $L_2FCs$ of all remaining transcripts in the same gene. A transcript was determined as significant DTU if it had adjusted p-value < `r params$DDD.data$params_list$pval_cut` and $\Delta$PS $\geq$ `r params$DDD.data$params_list$deltaPS_cut` (Figure 1).
...
if(!file.exists('figure/Distribution of the number of transcripts per gene.png')){
  cat('Plot is not found in the figure folder')
} else {
  knitr::include_graphics("figure/Distribution of the number of transcripts per gene.png")
}
```
...
if(!file.exists('figure/DE genes up and down regulation numbers.png')){
  cat('Plot is not found in the figure folder')
} else {
  knitr::include_graphics("figure/DE genes up and down regulation numbers.png")
}
```


```{r updown_DEtrans,eval=T,echo=F,fig.width=12}
if(!file.exists('figure/DE transcripts up and down regulation numbers.png')){
  cat('Plot is not found in the figure folder')
} else {
  knitr::include_graphics("figure/DE transcripts up and down regulation numbers.png")
}
```
...
```{r,eval=T,echo=F,fig.width=12,message=F,warning=F}
if(!file.exists('figure/Transcript expression distribution.png')){
  cat('Plot is not found in the figure folder')
} else {
  knitr::include_graphics("figure/Transcript expression distribution.png")
}
...
```{r,eval=T,echo=F,fig.width=12,message=F,warning=F}
if(!file.exists('figure/Gene expression distribution.png')){
  cat('Plot is not found in the figure folder')
} else {
  knitr::include_graphics("figure/Gene expression distribution.png")
}
```
