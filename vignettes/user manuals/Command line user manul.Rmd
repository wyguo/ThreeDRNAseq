...
knitr::opts_chunk$set(echo = T,eval = T,message = T, warning = F,
...
Only if see clear batch effects on the PCA plots, otherwise skip this step
```{r}
...
lfc.cutoff <- 1
##----->> limma pipeline
genes_DDD_stat <- limma.pipeline(dge = genes_dge,
                                 design = design,
                                 deltaPS = NULL,
                                 contrast = contrast,
                                 diffAS = F,
...
# genes_DDD_stat <- edgeR.pipeline(dge = genes_dge,
#                                  design = design,
#                                  deltaPS = NULL,
#                                  contrast = contrast,
#                                  diffAS = F,
...
## save results
save(genes_DDD_stat,file=paste0(data.folder,'/genes_DDD_stat.RData'))
...
deltaPS.cutoff <- 0.1
##----->> limma pipeline
trans_DDD_stat <- limma.pipeline(dge = trans_dge,
                                 design = design,
                                 deltaPS = deltaPS,
                                 contrast = contrast,
                                 diffAS = T,
...
##----->> edgeR glmQL pipeline
# trans_DDD_stat <- edgeR.pipeline(dge = trans_dge,
#                                  design = design,
#                                  deltaPS = deltaPS,
#                                  contrast = contrast,
#                                  diffAS = T,
...
## save results
save(trans_DDD_stat,file=paste0(data.folder,'/trans_DDD_stat.RData'))

```

## Result summary
### DDD target and tables of statistics
```{r}
################################################################################
##----->> Summary DE genes
...
if(DAS.p.method=='F-test') {
  DAS.stat <- trans_DDD_stat$DAS.F.stat
} else {
  DAS.stat <- trans_DDD_stat$DAS.Simes.stat
}
...
################################################################################
##----->> target numbers
summary.DDD.number(DE_genes = DE_genes,
                   DAS_genes = DAS_genes,
                   DE_trans = DE_trans,
                   DTU_trans=DTU_trans,
                   contrast = contrast)
